...
class Postgree {
    ...
    putEstadoInstancia = async (idInstanciaInteravel, idEstado) => {
        let resultados = [];
        await this.client.query(`UPDATE InstanciaInteravel SET EstadoAtual = ${idEstado} WHERE IdInstanciaInteravel = ${idInstanciaInteravel}`)
            .then((results) => {
            resultados = results.rows;
        });
        return resultados[0];
